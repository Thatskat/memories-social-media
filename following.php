...
<section class="container" style="margin-top: 7rem;">
  <div class="followingAccounts">
    ...
    $sql = "SELECT followers.followedUserid, followers.followerUserid, users.firstname, users.lastname, users.username, users.profilePic FROM followers INNER JOIN users ON followers.followedUserid=users.userid WHERE followerUserid=$userid;";
    $result = mysqli_query($conn, $sql);
    if(mysqli_num_rows($result)<=0){
      echo '<p style="margin-top:8rem; text-align:center;font-size:2rem;">you are following no other accounts<p>';
    } else {
      $output = "";
      while($row = mysqli_fetch_assoc($result)){
        $sql="SELECT * FROM followers WHERE followedUserid=$row[followedUserid];";
        if($results = mysqli_query($conn, $sql)){
          $accountFollowers = mysqli_num_rows($results);
        }
        $sql = "SELECT * FROM followers WHERE followerUserid=$row[followedUserid];";
        if($results = mysqli_query($conn, $sql)){
          $accountFollowing = mysqli_num_rows($results);
        }
        $output .= '<div class="card mb-3 m-auto" style="width: 50%;height:200px;">
        <div class="row g-0">
          <div class="col-md-4">
            <img src="'.$row['profilePic'].'" class="img-fluid rounded-start" alt="'.$row['username'].'" style="height:200px;">
          </div>
          <div class="col-md-8">
            <div class="card-body">
              <h5 class="card-title">'.$row['firstname'].' '.$row['lastname'] .'</h5>
              <p class="card-text"><a href="./profilepage.php?user='.$row['followedUserid'].'" class="link-brand underline">@' .$row['username'] .'</a></p>
              <p class="card-text"><small class="text-muted">followers - '.$accountFollowers .' following - ' .$accountFollowing. '</small></p>
            </div>
          </div>
          </div>
      </div>';
